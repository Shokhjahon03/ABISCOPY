...
export default function Chart3() {
  ...
  const fetchData = async () => {
    ...
    const resParameter = await DataService.get(endpoints.qdParameter);
  ...
  const fetchDataParams = async () => {
    try {
      ...
      const resWaterworkData = await DataService.get(endpoints.qualityData, {
        ...dataParams,
      });
    ...
    <Card>
      <CardHeader className="gap-3 d-flex flex-column justify-content-start">
        <h4 className="card-title mb-0 ">
          Качество воды в каналах у гидроузлов
        </h4>
