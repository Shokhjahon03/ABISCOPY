...
import {
  addcountryOriginAsync,
  deletecountryOrigin,
  editcountryOriginAsync,
  getcountryOriginAsync,
} from "../../slices/countryOrigin";
...
const CountryOrigin = () => {
  ...
  const handleAdd = async (e) => {
    ...
    const response = await addcountryOriginAsync(
      "country-origin",
      dataDocument
    );
  ...
  const handleEdite = async (e) => {
    ...
    const response = await editcountryOriginAsync(`country-origin`, {
      ...dataDocument,
      id: current?.id,
    });
    if (response?.errorCode) {
  ...
  const handleDelete = async () => {
    const response = await deletecountryOrigin(`country-origin/${current?.id}`);
    if (response?.errorCode) {
      toast.error("An error has occured!");
    } else {
      toast.success("Done successfully!");
  ...
  document.title = "Country origins | ABMC SCADA - Kibera Technology";
  return (
    <React.Fragment>
      {/* <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Страна происхождения" pageTitle="Директория" /> */}
      <Row>
        <Col lg={12}>
          <Card>
            <CardBody>
