...
const Manifacturer = () => {
  ...
  const handleAdd = async (e) => {
    ...
    const response = await addmanufacturerAsync("manufacturer", dataDocument);
  ...
  const handleEdite = async (e) => {
    ...
    const response = await editmanufacturerAsync(`manufacturer`, {
      ...dataDocument,
      id: current?.id,
    });
    if (response?.errorCode) {
  ...
  const handleDelete = async () => {
    const response = await deletemanufacturer(`manufacturer/${current?.id}`);
    if (response?.errorCode) {
      toast.error("An error has occured!");
    } else {
      toast.success("Done successfully!");
  ...
  document.title = "Manufacturers | ABMC SCADA - Kibera Technology";
  return (
    <React.Fragment>
      {/* <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Производитель" pageTitle="Директория" /> */}
      <Row>
        <Col lg={12}>
          <Card>
            <CardBody>
