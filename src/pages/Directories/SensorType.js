...
const SensorType = () => {
  ...
  const handleAdd = async (e) => {
    ...
    const response = await addsensorTypeAsync("sensor-type", dataDocument);
  ...
  const handleEdite = async (e) => {
    ...
    const response = await editsensorTypeAsync(`sensor-type`, {
      ...dataDocument,
      id: current?.id,
    });
    if (response?.errorCode) {
  ...
  const handleDelete = async () => {
    const response = await deletesensorType(`sensor-type/${current?.id}`);
    if (response?.errorCode) {
      toast.error("An error has occured!");
    } else {
      toast.success("Done successfully!");
  ...
  document.title = "Sensor Types | ABMC SCADA - Kibera Technology";
  return (
    <React.Fragment>
      {/* <div className="page-content">
                <Container fluid>
                    <BreadCrumb title="Тип датчика" pageTitle="Директория" /> */}
      <Row>
        <Col lg={12}>
          <Card>
            <CardBody>
