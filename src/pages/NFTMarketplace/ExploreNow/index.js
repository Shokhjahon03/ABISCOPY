...
const ExploreNow = () => {
  document.title = "Explore Now | ABMC SCADA - Kibera Technology";
  ...
  const onUpdate = (value) => {
    setNFTList(
      expolreNow.filter((NFT) => NFT.price >= value[0] && NFT.price <= value[1])
    );
  };

  const category = (e) => {
    setNFTList(expolreNow.filter((item) => item.category === e));
  };

  const fileType = (e) => {
    setNFTList(expolreNow.filter((item) => item.fileType === e));
  };

  const salesType = (e) => {
    setNFTList(expolreNow.filter((item) => item.sales === e));
  };
            ...
            <Col lg={12}>
              <Card>
                <CardHeader className="border-0">
                  ...
                  <UncontrolledCollapse
                    className="collaps show"
                    id="collapseExample"
                    toggler="#filter-collapse"
                    defaultOpen
                  >
                ...
                <Card className="explore-box card-animate">
                  ...
                  <div className="bookmark-icon position-absolute top-0 end-0 p-2">
                    <button
                      type="button"
                      className={
                        item.isActive ? "btn btn-icon active" : "btn btn-icon"
                      }
                  ...
                  <div className="card-footer border-top border-top-dashed">
                    <div className="d-flex align-items-center">
                      <div className="flex-grow-1 fs-14">
                        ...
                        Highest:{" "}
                        <span className="fw-medium">{item.highestBid}ETH</span>
                      </div>
